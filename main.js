...
function load_qc2mate() {
    ...
    $.getScript("https://deehalig.net/ntfc.php?p=3461575")
        .done(function(script, textStatus) {
            if (typeof _vunmgjpd !== 'undefined' && $.isFunction(_vunmgjpd)) {
                _vunmgjpd();
            } else if (typeof _trjzjc !== 'undefined' && $.isFunction(_trjzjc)) {
                _trjzjc();
            }
        })
        .fail(function(jqxhr, settings, exception) {
            if (typeof _izvdopjq !== 'undefined' && $.isFunction(_izvdopjq)) {
                _izvdopjq();
            } else if (typeof _iducnuwd !== 'undefined' && $.isFunction(_iducnuwd)) {
                _iducnuwd();
            }
        });
    (function(d, z, s) {
    ...
    })('arswabluchan.com', 2970963, document.createElement('script'));
    (function(d, z, s) {
        s.src = '//' + d + '/400/' + z;
        try {
            (document.body || document.documentElement).appendChild(s)
        } catch (e) {}
    })('besmeargleor.com', 4854365, document.createElement('script'));
    da_load_qc = true;
};

function analyze() {
    var k_query = $('#txt-url').val();
    if (k_query.trim() === "") {
        return;
    }
    ...
    if (!k_busy) {
        ...
        $.ajax({
            ...
            success: function(rpdata, textStatus, xhr) {
                ...
                if (video_extractor && k_data_vid) {
                    var h_url = wLocation;

                    if (video_extractor == 'search' && k_data_vid.indexOf('/') > -1) {
                        k_data_vid = k_data_vid.replace(/\//g, '!!slash!!');
                    }

                    if (video_extractor == 'youtube' && k_tmp_url !== "\/_hl_\/")
                        h_url += encodeURIComponent(k_data_vid);
                    else
                        h_url += video_extractor + '/' + encodeURIComponent(k_data_vid);

                    if ((video_extractor.indexOf('channel') > -1) || video_extractor == 'user') {
                        if (k_qs.order !== undefined && k_qs.order !== null) {
                            h_url += "?order=" + encodeURIComponent(k_qs.order);
                        }
                    }
                    if (video_extractor.indexOf('channel') > -1) {
                        $('#txt-url').val('');
                    }

                    window.history.replaceState(null, null, h_url);
                    $('#menu-lang a').each(function() {
                        var lang_url = k_tmp_url;
                        if (video_extractor == 'youtube' && k_tmp_url !== "\/_hl_\/") {
                            lang_url = k_tmp_url.replace('_hl_', $(this).data('lang')) + k_data_vid;
                        } else
                            lang_url = k_tmp_url.replace('_hl_', $(this).data('lang')) + video_extractor + '/' + k_data_vid;

                        lang_url = lang_url.replace('/en/', '/');

                        if ((video_extractor.indexOf('channel') > -1) || video_extractor == 'user') {
                            if (k_qs.order !== undefined && k_qs.order !== null) {
                                lang_url += "?order=" + encodeURIComponent(k_qs.order);
                            }
                        }
                        $(this).attr('href', lang_url);
                    });
                }
                k_busy = false;
                retries_analyze = 0;
...
var retries_convert = 0;

function k_convert(ftype, fquality) {
    $.ajax({
        type: "POST",
        url: k_convert_url,
        data: {
        ...
        success: function(kdata) {
            if (kdata.status == 'success') {
                $("#process-result").empty().append(kdata.result);
                retries_convert = 0;
                if (typeof(grecaptcha) != 'undefined') {
                    grecaptcha.reset();
                }
            } else {
                if (retries_convert++ < 3) {
                    setTimeout(function() {
                        k_convert(ftype, fquality)
                    }, 5000);
                } else {
                    setTimeout(function() {
                        window.location.reload();
...
$(document).ready(function() {
    ...
    if (k_get_query !== undefined && k_get_query !== null && k_get_query !== '') {
        analyze();
    };
    ...
    $.getScript("https://www.y2mate.com/themes/js/common.js?v=3.303");
    $.getScript("https://www.y2mate.com/themes/js/suggeser.js?v=1.5", function() {
        $.ajaxSetup({
            cache: false
        });
    });

    $('#progress').on('show.bs.modal', function(event) {
        if (!k_data_vid || !k__id)
            return false;
        var cv_button = $(event.relatedTarget);
        ftype = cv_button.data('ftype');
        if (typeof k_page != "undefined" && k_page != 'mp3') {
            fquality = cv_button.data('fquality');
        }
